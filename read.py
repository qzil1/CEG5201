...
if __name__ == '__main__':
    ...
    print(f"|{'':^20}|{'Measured MP Time':^99}|{'Measured Cumulative MP time':^99}|")
    print(f"|{'-' * 20}|{'-' * 99}|{'-' * 99}|")
    row = ''.join([f"{i:^24}|" for i in [j for j in range(1, 1+4)] * 2])
    print(f"|{'pair Index/Process':^20}|{row}")
    print(f"|{'-' * 20}|{('-' * 24 + '|') * 4}{('-' * 24 + '|') * 4}")
    for i in range(8):
        measured_MP_time_str = ''.join([f"{j:^24}|" for j in mult_result[0][i]])
        measured_cumulative_time = [sum([mult_result[0][j][k] for j in range(i + 1)]) for k in range(4)]
    ...
    for i in range(10):
        measured_MP_time = [sum([mult_result[i][j][k]for j in range(8)]) for k in range(4)]
        measured_MP_time_str = ''.join([f"{j:^24}|" for j in measured_MP_time])
        measured_MP_time_list.append(measured_MP_time)
        measured_cumulative_time = [sum([measured_MP_time_list[j][k] for j in range(i + 1)]) for k in range(4)]
        measured_cumulative_time_str = ''.join([f"{j:^24}|" for j in measured_cumulative_time])
        print(f"|{i:^20}|{measured_MP_time_str}{measured_cumulative_time_str}")